...
from settings import API_AUTH, API_URL, MAIN_ORG_ID, MANDATE, MANDATE_STARTIME
...
def get_objects(endpoint, limit=300, *args, **kwargs):
    ...
    return [obj for page in get_data_from_pager_api_gen(url, limit) for obj in page]
...
def copy_old_data():
        ...
        organization = storage.organization_storage.get_or_add_object(
            {
                "name": org["name"],
                "parser_names": (
                    "|".join(org["name_parser"].split(","))
                    if org["name_parser"]
                    else org["name"]
                ),
                "classification": (
                    org["classification"]
                    if org["classification"] in CLASSIFICATIONS
                    else "other"
                ),
                "gov_id": org["gov_id"],
                "acronym": org["acronym"],
    ...
    for membership in memberships:
        ...
        if (
            not membership["person"]
            or not membership["organization"]
        ...
        person = storage.people_storage.get_or_add_object(
            {
                ...
                "date_of_birth": (
                    person["birth_date"].split("T")[0] if person["birth_date"] else None
                ),
                "date_of_death": (
                    person["death_date"].split("T")[0] if person["death_date"] else None
                ),
                "districts": [
                    areas_map_ids[area_id] for area_id in person["districts"]
                ],
            }
        )
            ...
            storage.membership_storage.get_or_add_object(
                {
                    ...
                    "on_behalf_of": (
                        orgs_map_ids[membership["on_behalf_of"]]
