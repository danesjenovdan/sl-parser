...
TEST_TRANSCRIPT_URL = "https://www.dz-rs.si/wps/portal/Home/seje/evidenca/!ut/p/z1/04_Sj9CPykssy0xPLMnMz0vMAfIjo8zivSy9Hb283Q0N3E3dLQwCQ7z9g7w8nAwsPE31w9EUGAWZGgS6GDn5BhsYGwQHG-lHEaPfAAdwNCBOPx4FUfiNL8gNDQ11VFQEAF8pdGQ!/dz/d5/L2dBISEvZ0FBIS9nQSEh/?mandat=X&type=pmagdt&uid=299A42C202BE27F9C1258DD500429D0C"
TEST_TRANSCRIPT_URL = "https://www.dz-rs.si/wps/portal/Home/seje/evidenca/!ut/p/z1/04_Sj9CPykssy0xPLMnMz0vMAfIjo8zivSy9Hb283Q0N3E3dLQwCQ7z9g7w8nAwsPE31w9EUGAWZGgS6GDn5BhsYGwQHG-lHEaPfAAdwNCBOPx4FUfiNL8gNDQ11VFQEAF8pdGQ!/dz/d5/L2dBISEvZ0FBIS9nQSEh/?mandat=X&type=sz&uid=F6DA0A4C18E8D5BAC1258DD90034B0F1"
...
class SpeechParser(object):
    ...
    def parse_content(self, htree):
        ...
        try:
            ...
            output_text = htree.cssselect("form > div")[2]
        except:
            return
        ...
        for line in lines:
            ...
            if self.state == ParserState.META:
                ...
                if (
                    ...
                    or re.search(self.DATE_TIME_REGEX, line)
                ):
                    ...
                    if self.page_idx == 0:
                        ...
                        if "time" in time.keys():
                            self.update_session_start_time(time["time"])
    ...
    def parse_person_line(self, line_tree):
        ...
        if not line.strip():
            ...
            return
        if self.DEBUG:
            print("Trying to find speaker in line:", line)
        ...
        if name_candidate:
            ...
            try:
                ...
                if self.DEBUG:
                    ...
                    print(name_candidate[0])
                    print(name_candidate[0].text_content())
                    print(name_candidate[0].text)
                mister_or_madam_line = ""
                minister_line = ""
                print('fail find person with "name"', e)
                sentry_sdk.capture_message(
                    f"Find person regex fails with error {e}. Name candidate is: {name_candidate}"
                )
            if person_line:
                if self.is_valid_name(person_line):
                    speaker = person_line
                else:
                    if self.DEBUG:
                        print(f"Invalid name found: {person_line}")
    ...
    def get_time_from_line(self, line):
        ...
        if match:
            return {"time": self.get_time_from_match(match)}
        match = re.search(self.DATE_TIME_REGEX, line)
        if match:
            date_time_str = match.group(0)
            try:
                dt = datetime.strptime(date_time_str, "%d.%m.%Y %H:%M:%S")
                return {"time": dt.strftime("%H:%M")}
            except:
                pass
